...
export default function ChatPage() {
  ...
  const sidebar = <aside className={`flex h-full shrink-0 flex-col border-r border-slate-700/70 bg-[#0f172a] text-white shadow-2xl transition-[width] duration-300 ${historyCollapsed ? 'w-[72px]' : 'w-72'}`}>
    <div className="flex items-center gap-2 border-b border-slate-700/70 p-3"><button type="button" onClick={startNewChat} className={`flex h-11 flex-1 items-center rounded-xl bg-primary text-sm font-semibold text-white shadow-lg shadow-teal-950/30 transition hover:bg-teal-600 ${historyCollapsed ? 'justify-center px-0' : 'gap-2 px-4'}`} title="New Chat"><PlusIcon className="h-5 w-5 shrink-0" /><span className={historyCollapsed ? 'hidden' : ''}>New Chat</span></button>{!historyCollapsed && <button type="button" onClick={() => setHistoryCollapsed(true)} className="hidden h-10 w-10 shrink-0 items-center justify-center rounded-xl text-slate-400 transition hover:bg-slate-800 hover:text-white lg:flex" aria-label="Collapse chat history"><ChevronLeftIcon className="h-4 w-4" /></button>}</div>
    {historyCollapsed ? <button type="button" onClick={() => setHistoryCollapsed(false)} className="mx-auto mt-3 hidden h-10 w-10 items-center justify-center rounded-xl text-slate-400 transition hover:bg-slate-800 hover:text-white lg:flex" aria-label="Expand chat history"><ChevronRightIcon className="h-4 w-4" /></button> : <div className="min-h-0 flex-1 overflow-y-auto p-3"><p className="px-2 pb-3 pt-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-400">Conversations</p>{historyLoading && sessions.length === 0 ? <p className="px-2 py-5 text-sm text-slate-400">Loading history...</p> : null}{!historyLoading && sessions.length === 0 ? <p className="px-2 py-5 text-sm leading-6 text-slate-400">Your conversations will appear here after you send a message.</p> : null}<div className="space-y-1.5">{sessions.map((session) => <button key={session.id} type="button" onClick={() => openSession(session)} className={`group flex w-full items-center gap-2 rounded-xl px-3 py-3 text-left transition ${activeSession?.id === session.id ? 'bg-slate-700/90 text-white shadow-sm' : 'text-slate-300 hover:bg-slate-800 hover:text-white'}`}><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{session.title}</span><span className="mt-1.5 block text-[11px] text-slate-400">{displayTime(session.updated_at)}</span></span><span role="button" tabIndex={0} onClick={(event) => removeSession(event, session)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') removeSession(event, session); }} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-slate-400 opacity-0 transition hover:bg-slate-600 hover:text-rose-300 group-hover:opacity-100 focus:opacity-100" aria-label={`Delete ${session.title}`}><TrashIcon className="h-4 w-4" /></span></button>)}</div></div>}
  </aside>;
